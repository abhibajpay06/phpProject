...
if (isset($_POST['submit'])) {
    ...
    // Verify old password
    if ($reset_password['password'] == $old_password) {

        $update_password = mysqli_query(
            $conn,
            "UPDATE user_login SET password = '$new_password' WHERE id = '$id'"
        );

        if ($update_password) {
            showAlert('Password reset successfully', 'index');
        } else {
            showAlert('Something went wrong', 'reset_password');
        }

    } else {
        showAlert('Old password not matched, Try again', 'reset_password');
    }
}
?>
