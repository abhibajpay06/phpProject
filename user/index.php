...
// init configuration
$clientID = "428927292190-ktuqo32s00401vtp80i8ss0g4ilirkdl.apps.googleusercontent.com";
$clientSecret = "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";
$redirectUri = "https://www.samaandekho.com/callback";
...
if (isset($_POST['submit'])) {
    ...
    // ================= FETCH USER =================
    $result = userLogin($conn, $email, $action);

    if (mysqli_num_rows($result) == 0) {
        showAlert("Your account is not approved by admin yet.", "index");
        exit;
    }
    ...
    if (!empty($user['lockout_until']) && strtotime($user['lockout_until']) > time()) {
        ...
        showAlert("Account locked. Try again after $remaining minutes.", "index");
        exit;
    }
    ...
    if ($password === $user['password']) {
        ...
        mysqli_query($conn, "
            ...
            (user_id, email, number, ip_address, browser, operating_system, device_type, login_time)
            VALUES (
                '{$user['id']}',
                '{$user['email']}',
                '$number',
        ...
        showAlert('Login Successful', 'home');
        exit;
        ...
        if ($failed >= 5) {
            ...
            mysqli_query($conn, "
                UPDATE user_login 
                SET failed_login_attempts = $failed, lockout_until = '$lockUntil' 
                WHERE email = '$email'
            ");

            showAlert("Too many attempts. Account locked for 30 minutes.", "index");
            exit;
            ...
            showAlert("Invalid password. Attempt $failed of 5.", "index");
            exit;
        }
    }
}
        ...
        <div class="authentication-inner">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <!-- /Logo -->
              <h4 class="mb-2">Welcome to Infonix! 👋</h4>
