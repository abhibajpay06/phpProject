...
if (isset($_POST['submit'])) {
    ...
    foreach ($_FILES['documents']['name'] as $key => $name) {
        if ($_FILES['documents']['error'][$key] === 0) {
            ...
            $randomNumber = rand(1000000000, 9999999999); // 10-digit random number
			$ext = pathinfo($name, PATHINFO_EXTENSION); // PATHINFO_EXTENSION FOR FILE EXTENSION like jpg, pdf, etc
			$filename = $randomNumber . "." . $ext; // unique filename
	...
	// Convert to comma-separated list for database
    $documents = implode(',', $uploaded_files);
	
	$update_user_docs = mysqli_query($conn, "UPDATE `user_login` SET `documents` = '$documents' WHERE `id` = '$id'");
}
?>
                  ...
                  <table class="table">
					...
					<?php foreach($row as $fetch){?>
                      <tr>
                        ...
                        <td><?= $fetch['name'];?></td>
                        <td>
							<a href = "user_attendance_history.php?registration_no=<?= $fetch['registration_no']?>">Attendance History</a>
						</td>
                      </tr>
					 <?php } ?>
                    </tbody>
